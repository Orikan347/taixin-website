(function (root, factory) {
  ...
  // The published map is the reviewable source for every student-visible reply.
  let closingMap = null;
  ...
  function objections() {
    if (!closingMap || !Array.isArray(closingMap.objections)) return {};
    const mapped = {};
    closingMap.objections.forEach(function (node) {
      mapped[node.ask] = {
        ...
        responses: node.responses || null,
        follow_responses: node.follow_responses || [],
        response_protocol: node.response_protocol || [],
        buttons: node.buttons || [],
        proof_cards: closingMap.proof_cards || []
      };
    });
    return mapped;
  }
  ...
  function transition(state, raw) {
    ...
    if (next.phase === 'course_next_step') return answerCourseNextStep(next, text);
  ...
  function courseNextStep(state) {
    state.phase = 'course_next_step';
    return reply(state, dialogue('course_next_step'), ['我要報名課程', '找教育顧問聊一聊', '我還有一個疑慮', '看看其他課程'], profileSpec(state), coursePath(state));
  }

  function answerCourseNextStep(state, text) {
    if (text === '我還有一個疑慮') return objectionGate(state);
    if (text === '看看其他課程') return courseOverview(state);
    if (text === '我要報名課程' || text === '找教育顧問聊一聊') {
      return reply(state, dialogue('course_next_confirmation'), ['我還有一個疑慮', '看看其他課程', '課程介紹', '我的銷售天賦'], profileSpec(state), coursePath(state));
    }
    return courseNextStep(state);
  }
  ...
  function answerObjectionGate(state, text) {
    ...
    const disc = discResult(state).primary;
    const answer = item.responses && item.responses[disc] || item.reply_by_disc && item.reply_by_disc[disc] || '我先把你在意的地方講清楚。';
    return reply(state, answer, item.follow.concat('我都清楚了'));
  }

  function answerObjectionDetail(state, text) {
    if (text === '我都清楚了' || text === '這樣有回答我的問題' || text === '依我的情況推薦課程') return recommendation(state);
    if (text === '我還有一個疑慮') return objectionGate(state);
    if (text === '課程介紹') return courseOverview(state);
    const item = objections()[state.activeObjection];
    if (!item || item.follow.indexOf(text) === -1) return answerObjectionGate(state, state.activeObjection);
    ...
    if (!answer) return reply(state, '我先把這一點補清楚，再由你決定要不要往下看。', ['我還有一個疑慮', '課程介紹', '我的銷售天賦', '依我的情況推薦課程']);
    return reply(state, answer, ['這樣有回答我的問題', '我還有一個疑慮', '課程介紹', '依我的情況推薦課程']);
  }
  ...
  return { start: start, transition: transition, configure: configure, COURSE_LINKS: COURSE_LINKS, COURSE_TREE: COURSE_TREE };
});
