(function (root, factory) {
  ...
  function transition(state, message) {
    ...
    if (next.phase !== 'discovery' && next.phase !== 'explanation') {
    ...
    if (next.phase === 'course_intro') {
      if (/流量磁鐵/.test(text)) return buildCourseDetailResponse(next, 'liuliang');
      if (/成交地圖/.test(text)) return buildCourseDetailResponse(next, 'chengjiao');
      if (/直指人心/.test(text)) return buildCourseDetailResponse(next, 'zhizhi');
      if (/極致效率/.test(text)) return buildCourseDetailResponse(next, 'xiaolu');
      if (/言之有物/.test(text)) return buildCourseDetailResponse(next, 'yanzhi');
      if (/其餘三堂|其他三堂|看其他/.test(text)) return buildOtherCoursesResponse(next);
      if (/怎麼選課|回到五堂|五堂課介紹/.test(text)) return buildCourseSelectionResponse(next);
      if (/客戶不夠多/.test(text)) return buildCourseDetailResponse(next, 'liuliang');
      if (/成交卡住/.test(text)) return buildCourseDetailResponse(next, 'chengjiao');
      if (/講不清價值/.test(text)) return buildCourseDetailResponse(next, 'yanzhi');
      if (/天賦|報告/.test(text)) return buildReportResponse(next);
      if (isClear(text)) return buildCourseCompletionResponse(next);
      return formatResponse(next, '你可以選下面想看的方向；如果你已經清楚了，直接按「我都清楚了」。', currentButtons(next));
    }
  ...
  function currentButtons(state) {
    ...
    if (state.phase === 'course_intro') return courseIntroButtons();
  ...
  function buildCourseContentResponse(state) {
    state.phase = 'course_intro';
    const intro = Object.values(COURSE_CATALOG).map(formatCourseBlock).join('\n\n');
    const reply = `我把五堂課都整理給你。它們不是五個零散工具，而是從名單、成交、看懂客戶、時間，到表達和帶人，把銷售能力一段一段補齊。你可以先從現在最想解決的地方開始，不需要一次全部學。\n\n${intro}\n\n你想先看哪一堂的詳細內容？\n\n以上的說明都清楚嗎？`;
    return formatResponse(state, reply, currentButtons(state));
  }

  function buildCourseSelectionResponse(state) {
    const reply = '選課先看你現在最想處理哪一段：\n\n- 客戶不夠多、開發沒有方向：流量磁鐵。\n- 客戶來了卻成交卡住：成交地圖。\n- 看不懂客戶、信任建立不夠：直指人心。\n- 每天很忙、資料和追蹤很亂：極致效率。\n- 講不清價值、需要表達和帶人：言之有物。\n\n以上的說明都清楚嗎？';
    return formatResponse(state, reply, courseIntroButtons());
  }

  function courseIntroButtons() {
    return ['看流量磁鐵詳細內容', '看成交地圖詳細內容', '看其餘三堂課', '我都清楚了'];
  }
  ...
  function buildCourseDetailResponse(state, courseId) {
    ...
    state.phase = 'course_intro';
    const reply = `${formatCourseBlock(course)}\n\n所以，如果你現在最想處理的是「${course.benefit.replace('幫你處理「', '').replace('」。', '')}」，這堂會是很直接的開始。\n\n以上的說明都清楚嗎？`;
    return formatResponse(state, reply, ['回到五堂課介紹', '找出我的銷售天賦', '看開課時間費用', '我都清楚了']);
  }

  function buildOtherCoursesResponse(state) {
    state.phase = 'course_intro';
    const courses = ['zhizhi', 'xiaolu', 'yanzhi'].map((id) => formatCourseBlock(COURSE_CATALOG[id])).join('\n\n');
    const reply = `這三堂分別處理「看懂人」、「用好時間」和「把話說清楚」。\n\n${courses}\n\n以上的說明都清楚嗎？`;
    return formatResponse(state, reply, ['看流量磁鐵詳細內容', '看成交地圖詳細內容', '課程怎麼選', '我都清楚了']);
  }
