(function (root, factory) {
  ...
  function courseTree() {
    ...
    closingMap.course_nodes.forEach(function (node) {
      ...
      if (id) tree[id].questions.push([node.ask, node.base_answer, node.id]);
    });
    return tree;
  }
  ...
  function start(profile) {
    ...
    return reply(state, `${name}，你好！我先幫你把現在的銷售狀況抓出來。接下來會問你三個跟你有關的問題，照平常的反應選就好，最後我會整理成一份你帶得走的銷售天賦報告。\n\n${questions[0].prompt}`, labels(questions[0]));
  }
  ...
  function talentStart(state) {
    ...
    const questions = talentQuestions();
    return reply(state, `好，我們重新從你自己的做事方式開始看。\n\n${questions[0].prompt}`, labels(questions[0]));
  }
  ...
  function answerTalent(state, text) {
    ...
    if (!option) return reply(state, `這題是要看你自己的習慣，選一個最像你的就好。\n\n${question.prompt}`, labels(question));
    ...
    return reply(state, `${state.profile.name || '你'}，我把你的銷售天賦整理成表格，已經放在底下給你囉。\n\n你覺得有準嗎？想繼續了解怎麼讓自己成長更快，回答 yes 就好；也可以直接看課程介紹。`, ['課程介紹', '推薦我的課程', '我的銷售天賦', '我都清楚了'], report, path);
  }
  ...
  function reportMenu(state, text) {
    ...
    return reply(state, '你可以先看五堂課各自幫你解決什麼，或讓我依你面對的客戶，排出最適合的學習順序。', ['課程介紹', '推薦我的課程', '我的銷售天賦', '我都清楚了'], profileSpec(state), defaultPath(state));
  }

  function audienceQuestion(state) {
    state.phase = 'course_audience';
    return reply(state, '我會依你面對的對象，幫你把學習順序排得更準。\n\n客戶通常是哪一種人？', ['上班族或家庭客戶', '企業老闆或主管', '高價專業服務客戶', '招募團隊或尋找合作夥伴']);
  }
  ...
  function answerAudience(state, text) {
    ...
    return reply(state, `了解。面對${text}時，最近一次對方沒有往前走，最像哪一種情況？`, options);
  }
  ...
  function answerSituation(state, text) {
    ...
    state.expectedButtons = options;
    return reply(state, `你剛剛說「${text}」。下次遇到同樣狀況，你最想先補強哪一段？`, options);
  }
  ...
  function answerSkill(state, text) {
    ...
    return reply(state, `${state.profile.name || '你'}，我會先讓你從最能處理「${text}」的課開始，後面兩堂接著幫你把方法做完整。建議順序已經放在底下。`, ['課程介紹', '我的銷售天賦', '我要看開課資訊', '我都清楚了'], profileSpec(state), path);
  }
  ...
  function courseOverview(state) {
    ...
    return reply(state, `我先把五堂課各自能幫你的地方講清楚。你看到有興趣的，直接點進去；每堂課裡都有三個大家最常問的問題。\n\n${list}\n\n你想先了解哪一堂？`, Object.keys(courses).map(function (id) { return courses[id].name; }));
  }
  ...
  function chooseCourse(state, text) {
    ...
    return reply(state, `${course.name}，我先從你最在意的結果開始講。你想先從哪一段了解？`, detailButtons(state));
  }
  ...
  function answerCourseDetail(state, text) {
    ...
    const node = course.questions.find(function (item) { return item[0] === text; });
    ...
    const replyText = courseLead(studentDisc, state, course.name) + '\n\n' + node[1] + '\n\n' + courseClose(studentDisc, course.name);
    return reply(state, replyText, detailButtons(state));
  }

  function detailButtons(state) {
    const course = courseTree()[state.activeCourse];
    const remaining = course.questions.filter(function (item) { return state.answeredCourseNodes.indexOf(item[0]) === -1; }).map(function (item) { return item[0]; });
  ...
  function objectionGate(state) {
    ...
    return reply(state, '五堂課你都看過主重點了。真的要做決定前，如果心裡還卡著一件事，直接點出來就好，我幫你把它講清楚。', Object.keys(objections()).concat('沒有疑慮了'));
  }
  ...
  function answerObjectionDetail(state, text) {
    ...
    const answer = objectionFollowUp(item.key, text, discResult(state).primary, course);
  ...
  function recommendation(state) {
    ...
    return reply(state, `${state.profile.name || '你'}，先從第一堂開始就好。先把現在最想解決的問題處理好，後面兩堂會讓你把方法接得更完整。學習順序和課程入口已經放在底下。`, ['我要看開課資訊', '我要報名', 'LINE 顧問', '課程介紹'], profileSpec(state), path);
  }
  ...
  function objectionReply(key, disc, state, course) {
    ...
    const answer = objection && objection.reply_by_disc && objection.reply_by_disc[disc] ? objection.reply_by_disc[disc] : variants[key][disc];
