(function (root, factory) {
  ...
  function transition(state, message) {
    ...
    if (next.phase === 'course_intro') {
      if (/怎麼選課/.test(text)) return buildCourseSelectionResponse(next);
      if (/客戶不夠多/.test(text)) return buildCourseFocusResponse(next, 'liuliang');
      if (/成交卡住/.test(text)) return buildCourseFocusResponse(next, 'chengjiao');
      if (/講不清價值/.test(text)) return buildCourseFocusResponse(next, 'yanzhi');
      if (/其他課程/.test(text)) return buildCourseContentResponse(next);
      if (/天賦|報告/.test(text)) return buildReportResponse(next);
  ...
  function currentButtons(state) {
    ...
    if (state.phase === 'course_intro') return ['課程怎麼選', '找出我的銷售天賦', '看開課時間費用', '我都清楚了'];
  ...
  function buildCourseContentResponse(state) {
    ...
    const intro = Object.values(COURSE_CATALOG)
      .map((course) => `- ${course.name}：${course.benefit}`)
      .join('\n');
    const reply = `我用利益點跟你講就好，不講太細。\n\n${intro}\n\n你想知道自己該先補哪一段，也可以直接做銷售天賦報告。\n\n以上的說明都清楚嗎？`;
    return formatResponse(state, reply, currentButtons(state));
  }

  function buildCourseSelectionResponse(state) {
    const reply = '選課先看你現在最想解決哪一件事：\n\n- 客戶不夠多：先看流量磁鐵。\n- 成交卡住：先看成交地圖。\n- 講不清價值：先看言之有物。\n\n以上的說明都清楚嗎？';
    return formatResponse(state, reply, ['客戶不夠多', '成交卡住', '講不清價值', '我都清楚了']);
  }

  function buildCourseFocusResponse(state, courseId) {
    const course = COURSE_CATALOG[courseId];
    const reply = `${course.name}會先幫你處理「${course.benefit.replace('幫你處理「', '').replace('」。', '')}」。${course.reason}\n\n以上的說明都清楚嗎？`;
    return formatResponse(state, reply, ['課程怎麼選', '找出我的銷售天賦', '看開課時間費用', '我都清楚了']);
  }
