(function (root, factory) {
  ...
  const COURSE_CATALOG = {
    ...
    xiaolu: {
      ...
      solves: ['很忙', '沒時間', '資料散亂', '忘記追蹤'],
      reason: '建立客戶資料、時間與追蹤系統，讓努力花在會產生結果的位置。'
    },
    yanzhi: {
      id: 'yanzhi',
      name: '言之有物',
  ...
  const TALENTS = {
    ...
    follow_up_system: {
      label: '系統追蹤型',
      description: '你適合建立客戶資料、追蹤節奏與時間系統，讓努力不漏接。',
  ...
  const OFFERING_REPLY = '已確認的開課時間與金額我會以頁面「開課時間與金額」區塊為準，不在聊天裡亂猜。複訓費用已確認：一律 NT$ 3,200。吉隆坡「超級銷冠系統」目前日期已確認為 2026/10/21-10/23，金額待確認；名額未提供，所以不顯示。';
  ...
  function computeLifeNumbers(birthdate) {
    ...
    if (!match) {
      return { talent_number: null, life_number: null, birthday_number: null };
    }
  ...
  function applyProfileScores(state) {
    ...
    applyExplicitDiscSignal(state, state.profile.disc_hate_customer, '客戶互動最卡住的點');
  ...
  function applySignals(state, text) {
    const signalMap = [
      ...
      { keys: ['很忙', '效率', '時間', '資料', '追蹤', '忘記'], talent: 'follow_up_system', disc: 'C', blocker: '追蹤系統不足' },
  ...
  function buildToneIntro(strategy, talent, blocker) {
    ...
    if (strategy.disc_primary === 'S') {
      return `我先跟你說，這不是你的問題。你比較像需要安全感和節奏的人，所以銷售上不能逼自己變得很強勢。`;
    }
  ...
  function buildBestStrategy(talentId, disc) {
    const byTalent = {
      ...
      objection_handling: '先理解對方卡在哪裡，再證實你的方案能處理那個疑慮。',
  ...
  function buildRecommendationReply(state) {
    ...
    return `那如果這個方向是準的，我會建議你不要一次什麼都學，而是照一個順序補。這樣你會進步得更快。\n\n${lines}\n\n目前已確認的場次與金額我整理在頁面上；吉隆坡場金額待確認，複訓費用是 NT$ 3,200。`;
  }
  ...
  function transition(state, message) {
    ...
    if (next.phase === 'revise_profile') {
      ...
      return formatResponse(next, '好，我們先補充你的真實情境。我想修正一下判斷：你覺得剛剛最不像你的地方，是行為模式、銷售天賦，還是我抓錯你的成交情境？', '你覺得最不像的是哪一塊？');
    }
