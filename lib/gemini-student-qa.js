(function (root, factory) {
  ...
  function buildSystemPrompt(state, deterministicResult, userMessage) {
    ...
    return [
      ...
      '',
      '階段規則：',
      '- 基本資料不算對話。至少要有 3 則有效使用者回答，而且必須取得：背景/產業、商品/服務、客戶對象、最近遇到的銷售問題。缺任一項只能 discovery 追問，不可 profile_ready。',
      '- 前 3 輪要問跟學生回答有關的問題，不能重複問同一題。',
      '- 如果還不知道他的商品、客戶、最近遇到的情境，優先追問這三件事。',
  ...
  function normalizeGeminiResult(state, result) {
    ...
    if (normalized.profile_spec && (!discovery.can_build_report || !isCompleteProfileSpec(normalized.profile_spec))) {
      const question = getNextDiscoveryQuestion(discovery);
      return {
        phase: PHASES.DISCOVERY,
        reply: `我還差一點點就能幫你整理報告。${question}`,
  ...
  function normalizeProfileSpec(spec) {
    ...
    return {
      ...
      student_strength: String(spec.student_strength || ''),
  ...
  function fallbackReply(state, commercialGuard) {
    ...
    const discovery = analyzeDiscoveryEvidence(messages, state.profile || {});
    const userTurns = discovery.effective_turns;
    const deterministic = getDeterministicEngine();
    ...
    if (state.recommendation_allowed) {
      return {
        ...
        reply: '太好了，這代表你其實不是沒有能力，而是下一步要把優勢整理成方法。照這個學習順序補起來，你會更快知道自己每一次跟客戶談話該做什麼。',
        next_question: '',
        profile_spec: null,
  ...
  function buildFallbackProfile(state, spec, life) {
    ...
    return {
      disc_type: disc,
      life_number: computed.life_number ? String(computed.life_number) : (life ? String(life).slice(-1) : ''),
      sales_advantages: buildDiscAdvantages(disc),
      life_talents: buildLifeTalents(computed),
      student_strength: '你很適合先建立信任，再用問題找出真正原因。',
  ...
  function buildLifeTalents(computed) {
    const numbers = [computed.life_number, computed.birthday_number].filter(Boolean);
    const activeLine = Array.isArray(computed.active_lines) && computed.active_lines.length ? computed.active_lines[0] : null;
    const items = numbers.slice(0, 2).map((number) => lifeTalentFor(number));
  ...
  function lifeTalentFor(number) {
    const map = {
      ...
      5: ['5 號：變通力', '你反應快，能在不同客戶面前調整說法。', '銷售上適合處理臨場異議，把停住的對話轉成新角度。'],
  ...
  function analyzeDiscoveryEvidence(messages, profile) {
    ...
    const allText = normalizeForEvidence([profile && profile.industry, profile && profile.role, profile && profile.problems && profile.problems.join(' '), profile && profile.goals && profile.goals.join(' '), effectiveMessages.join(' ')].filter(Boolean).join(' '));
    const userText = normalizeForEvidence(effectiveMessages.join(' '));
    const coverage = {
      background: Boolean(profile && (profile.industry || profile.role)) || /(我是|目前|產業|業務|顧問|主管|老闆|經營|銷售)/.test(userText),
      product: /(賣|做|商品|產品|服務|課程|方案|顧問|保險|房|車|醫美|美容|投資|教練|系統|工具)/.test(userText),
      customer: /(客戶|對象|企業主|家庭|媽媽|爸爸|主管|老闆|學生|學員|公司|消費者|買家|會員|團隊)/.test(userText),
      problem: /(太貴|再想想|考慮|拒絕|異議|成交|信任|追蹤|已讀|沒回|很忙|效率|名單|沒結果|看不懂|講不清楚|猶豫|預算|比較|問題)/.test(allText)
    };
    ...
    return {
      ...
      coverage,
      missing,
      can_build_report: effectiveMessages.length >= 3 && missing.length === 0,
  ...
  function getNextDiscoveryQuestion(discovery) {
    const evidence = discovery || { effective_turns: 0, missing: [] };
    if (evidence.missing && (evidence.missing.includes('product') || evidence.missing.includes('customer'))) {
      return '你先跟我說清楚一點：你現在主要賣什麼？客戶通常是哪一種人？';
    }
    if (evidence.missing && evidence.missing.includes('problem')) {
      return '最近一次沒有成交，對方當時怎麼回你？是價格、信任、需求，還是後續追蹤出了問題？';
    }
    if (evidence.effective_turns < 2) {
      return '你再補一個真實例子：最近有哪個客戶讓你印象最深？他怎麼問、你怎麼回答？';
    }
    return '最後我再確認一題：如果同樣的客戶下次再出現，你最希望自己哪一個地方變得更強？';
  }
