(function (root, factory) {
  ...
  function chooseConsultativeNextQuestion(slots, quote) {
    ...
    if (!slots.goal) return '你最想先改善成交、信任、追蹤、表達價值，還是每天太忙沒結果？';
    if (/成交|買|下單/.test(quote)) return '這次如果要重來，你最想把哪一句話講得更有力量？';
    return '如果同樣狀況下次再出現，你最希望自己哪一個地方變得更強？';
  }
  ...
  function buildDeterministicProfileReady(state) {
    ...
    return {
      ...
      reply: `${state.profile && state.profile.name ? state.profile.name + '，' : ''}${quoteLead}我已經把你的銷售現場整理好了。你不是沒有能力，而是要把自己最自然的成交方式放大。\n\n你看完之後，覺得像你嗎？有準嗎？`,
      next_question: '',
