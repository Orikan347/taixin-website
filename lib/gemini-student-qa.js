(function (root, factory) {
  ...
  function buildDeterministicResponse(state) {
    ...
    if (/(時間|日期|開課|多少錢|價格|費用|名額|複訓|重修|課程學什麼|會學到什麼)/.test(latest)) {
      return safeCommercialNotice(state);
    }
    if (state.recommendation_allowed) {
      return {
        ...
        next_question: '',
        profile_spec: null,
        course_path: defaultCoursePath(state),
        cta_ready: true
      };
    }
    const discovery = analyzeDiscoveryEvidence(state.messages || [], state.profile || {});
  ...
  function chooseDeterministicQuestion(slots) {
    ...
    if (!slots.problem) {
      return `好，我抓到了，你賣的是${slots.product}，對象是${slots.customer}。最近一次沒有往前走，對方是卡在價格、信任，還是一直說再想想？`;
    }
    if (!slots.goal) {
      return `我有抓到你遇到的狀況了。那如果下一次遇到同樣客戶，你最想讓自己哪個地方變強？成交、信任、追蹤，還是表達價值？`;
    }
    return `你前面講的我都記下來了。再補一句就好：你最想讓這份報告幫你看見哪一個銷售優勢？`;
  }
  ...
  function buildLifeTalents(computed) {
    ...
    const activeLine = computed && Array.isArray(computed.active_lines) && computed.active_lines.length ? computed.active_lines[0] : '';
