(function (root, factory) {
  ...
  function buildSystemPrompt(state, deterministicResult, userMessage) {
    ...
    return [
      ...
      '- 每一輪 discovery 回覆都必須先聽學生最新原話：用 1 句承接他剛剛講的詞，再判斷表面問題與深層問題，接著用泰欣方法回他，最後只問 1 個和他原話有關的下一題。',
      '- Discovery 回覆固定結構：我聽到你剛剛說「學生原話」 -> 這表面上是____，真正要看的是____ -> 用泰欣的 MBAF / BAF / 五連問 / 拒絕處理 / 言之有物 / 極致效率其中一個方法 -> 下一題。',
      '- 不可把同一句 discovery 問題連續問兩次。學生已提供真實客戶原話後，不可再問「最後補一個真實情境」或「原話大概怎麼說」。',
      ...
      '- 當對話蒐集狀態 can_build_report=true，phase 才能設為 profile_ready，產出報告，reply 要自然說：我把你的銷售天賦整理成表格，已經放在底下給你囉。你覺得有準嗎？也可以繼續了解怎麼做，讓自己成長更快。想繼續了解，直接回 yes。',
  ...
  function normalizeGeminiResult(state, result) {
    ...
    if (normalized.profile_spec && (!discovery.can_build_report || !isCompleteProfileSpec(normalized.profile_spec))) {
      return discovery.can_build_report
        ...
        : buildConsultativeResponse(state);
    }
    if (discovery.can_build_report && !normalized.profile_spec && normalized.phase !== PHASES.RECOMMENDATION) {
      return buildDeterministicProfileReady(state);
    }
    ...
    if (shouldUseConsultativeResponse(state, normalized, discovery)) {
      return buildConsultativeResponse(state);
    }
  ...
  function buildDeterministicResponse(state) {
    ...
    const discovery = analyzeDiscoveryEvidence(state.messages || [], state.profile || {});
    return discovery.can_build_report ? buildDeterministicProfileReady(state) : buildConsultativeResponse(state);
  }
  ...
  function buildAgreementRecommendation(state) {
    ...
    const lines = path.slice(0, 3).map((course, index) => {
      const name = stripDisplayText(course && course.name || '');
      const reason = stripDisplayText(course && course.reason || '');
  ...
  function chooseDeterministicQuestion(slots) {
    ...
    return `我已經抓到方向了。再補一句最近客戶實際講過的話，像「太貴」「再比較」「沒時間」這種原句，我就能把報告整理得更像你。`;
  }

  function shouldUseConsultativeResponse(state, normalized, discovery) {
    if (!normalized || normalized.profile_spec || normalized.phase === PHASES.RECOMMENDATION) return false;
    if (discovery && discovery.can_build_report) return false;
    const quote = latestMeaningfulUserQuote(state);
    if (!quote) return false;
    const reply = stripDisplayText(normalized.reply || '');
    if (!reply) return true;
    if (isGenericDiscoveryQuestion(reply) || isRepeatedFinalScenarioQuestion(reply)) return true;
    if (isReplyExactRepeat(state, reply)) return true;
    if (/[？?]/.test(reply) && !replyReferencesQuote(reply, quote)) return true;
    return false;
  }

  function buildConsultativeResponse(state) {
    const slots = state.discovery_slots || extractDiscoverySlots(state.messages || [], state.profile || {});
    const quote = latestMeaningfulUserQuote(state);
    if (!quote) return buildDeterministicQuestion(state);
    const staged = buildStagedSlotResponse(state, slots, quote);
    if (staged) return staged;
    const diagnosis = diagnoseSalesUtterance(quote, slots);
    const name = state.profile && state.profile.name ? `${stripDisplayText(state.profile.name)}，` : '';
    const reply = [
      `${name}我有聽到你剛剛說「${quote}」。${diagnosis.surface}`,
      diagnosis.deep,
      `用泰欣的方法，${diagnosis.method}${diagnosis.tip ? `。${diagnosis.tip}` : ''}`,
      `我再問你一個關鍵就好：${diagnosis.next_question}`
    ].filter(Boolean).join('\n\n');
    return {
      phase: PHASES.DISCOVERY,
      reply,
      next_question: diagnosis.next_question,
      profile_spec: null,
      course_path: [],
      cta_ready: false
    };
  ...
  function buildStagedSlotResponse(state, slots, quote) {
    const product = stripDisplayText(slots.product || '');
    const customer = stripDisplayText(slots.customer || '');
    const problem = stripDisplayText(slots.problem || '');
    const goal = stripDisplayText(slots.goal || '');
    const name = state.profile && state.profile.name ? `${stripDisplayText(state.profile.name)}，` : '';
    const latestSlot = inferLatestAnswerSlot(state, slots, quote);
    const response = (reply, nextQuestion) => ({
      phase: PHASES.DISCOVERY,
      reply,
    ...
    if (problem && isSalesSituationQuote(quote)) return null;

    if (product && !customer) {
      const nextQuestion = `通常會買${product}的是哪一種人？`;
      return response(
        `${name}我抓到你賣的是「${product}」。先不用講太細，我要先知道誰會買，才判斷你該用信任、故事、專業，還是效率去成交。\n\n我再問你一個關鍵就好：${nextQuestion}`,
        nextQuestion
      );
    }

    if (product && customer && !problem) {
      const nextQuestion = `${customer}最常不買，是覺得沒時間、價格高、看不到效果，還是先說再想想？`;
      const lead = latestSlot === 'customer'
        ? `${name}我抓到你賣的是「${product}」，對象是「${customer}」。這樣方向就清楚很多了。`
        : `${name}你剛剛補的是「${quote}」。我會把它放進你賣「${product}」、面對「${customer}」的情境裡看。`;
      return response(
        `${lead}\n\n我再問你一個關鍵就好：${nextQuestion}`,
        nextQuestion
      );
    ...
    if (product && customer && problem && !goal) {
      const nextQuestion = '你現在最想先改善成交率、建立信任、追蹤效率，還是把價值講清楚？';
      const lead = latestSlot === 'problem'
        ? `${name}我抓到問題了：「${quote}」。這代表對方不是完全沒興趣，是還有顧慮沒被問出來。`
        : `${name}你現在的情境我抓到了：賣「${product}」，對象是「${customer}」，目前遇到的是「${problem}」。`;
      return response(
        `${lead}\n\n用泰欣的方法，先用五連問把背後原因問出來，再用 BAF 把價值講成對方聽得懂的好處。\n\n我再問你一個關鍵就好：${nextQuestion}`,
        nextQuestion
      );
    }

    if (product && customer && !problem && goal) {
      const nextQuestion = `${customer}最近最常沒有往前走的理由是什麼？例如沒時間、太貴、再想想、看不到效果。`;
      return response(
        `${name}我抓到你想先改善「${goal}」。方向對，但我還要知道真實阻力，才不會把你的天賦報告寫成空話。\n\n我再問你一個關鍵就好：${nextQuestion}`,
        nextQuestion
      );
    }
  ...
  function isSalesSituationQuote(text) {
    return /(沒錢|太貴|預算|價格|很貴|高價|再比較|比較|看看別家|比價|家人|另一半|先生|太太|主管|老闆|討論|問一下|沒有效|效果|怕|擔心|不確定|沒把握|風險|怕買錯|沒時間|很忙|太忙|忙|來不及|懶得|沒空|再想想|考慮|想一下|晚點|之後再說)/.test(stripDisplayText(text));
  }

  function inferLatestAnswerSlot(state, slots, quote) {
    const asked = slots && slots.last_asked_slot ? slots.last_asked_slot : '';
    const value = stripDisplayText(quote);
    if (asked) return asked;
    if (slots && slots.product && normalizeForEvidence(slots.product) === normalizeForEvidence(value)) return 'product';
    if (slots && slots.customer && normalizeForEvidence(slots.customer) === normalizeForEvidence(value)) return 'customer';
    if (extractProblem(value)) return 'problem';
    if (extractGoal(value)) return 'goal';
    return '';
  }

  function diagnoseSalesUtterance(text, slots) {
    const quote = stripDisplayText(text);
    const product = slots.product || '你的產品';
    const customer = slots.customer || '客戶';
    if (/沒錢|太貴|預算|價格|很貴|高價/.test(quote)) {
      return {
        surface: '這表面上是價格問題。',
        deep: `但我會先把它看成：${customer}還沒有把${product}跟他最想要的結果連在一起，所以他只看見「要花錢」，還沒看見「為什麼值得」。`,
        method: '先用五連問把「沒錢」問深，再用 MBAF 把價值講回他的利益',
        tip: '你不要急著解釋規格，可以先問他：「如果這個問題三個月後還是一樣，對你最大的影響是什麼？」',
        next_question: '他說沒錢時，比較像真的沒有預算，還是還沒覺得值得？'
      };
    }
    if (/再比較|比較|看看別家|比價/.test(quote)) {
      return {
        surface: '這表面上是客戶想比較。',
        deep: `真正要看的是：${customer}還沒有分清楚你跟其他選項的差異，所以他只好回到價格或品牌來比。`,
        method: '用「言之有物」把差異講成故事和畫面，再用 BAF 先講好處、再補特色',
        tip: '你可以少講一點你有多好，多講一點客戶用了之後會少掉什麼麻煩、得到什麼結果',
        next_question: '他說再比較時，通常拿你跟誰比？價格、品牌，還是服務內容？'
      };
    }
    if (/家人|另一半|先生|太太|主管|老闆|討論|問一下/.test(quote)) {
      return {
        surface: '這表面上是他要回去討論。',
        deep: '真正要看的是：現場那個人可能不是唯一決策者，或他還沒有足夠的理由幫你說服另一個人。',
        method: '用拒絕處理先同理，再問出背後決策者在意什麼，最後幫他準備一句帶得回家的理由',
        tip: '不要直接逼他決定，可以問：「你回去討論時，對方最可能先問哪一個問題？」',
        next_question: '他要討論的那個人，通常最在意價格、風險，還是效果？'
      };
    }
    if (/沒有效|效果|怕|擔心|不確定|沒把握|風險|怕買錯/.test(quote)) {
      return {
        surface: '這表面上是客戶擔心效果。',
        deep: `我會把它看成信任還沒完全建立。${customer}不是不想買，而是怕自己買錯、怕承諾落空。`,
        method: '先同理他的擔心，再用「言之有物」把案例講得有畫面，讓他看見結果不是空口說白話',
        tip: '你可以準備一個跟他背景很像的案例，讓他不是聽懂產品，而是看見自己也可能變好',
        next_question: '他最怕的是沒有效、怕被騙，還是怕買了之後沒有人帶他做？'
      };
    }
    if (/沒時間|很忙|太忙|忙|來不及|懶得|沒空/.test(quote)) {
      return {
        surface: '這表面上是時間問題。',
        deep: '但真正要看的是：這件事在他心中還沒有排到前面，或你的追蹤方式還沒有幫他省時間。',
        method: '用極致效率把下一步變得很清楚，讓客戶不用想太多也知道該怎麼往前走',
        tip: '你可以把下一步縮小成一個很容易答應的動作，不要一次丟太多資訊',
        next_question: '他說沒時間時，是沒時間了解，還是你後續追蹤太散，讓他不知道下一步是什麼？'
      };
    }
    if (/再想想|考慮|想一下|晚點|之後再說/.test(quote)) {
      return {
        surface: '這表面上是客戶想延後決定。',
        deep: '真正要看的是：他心裡還有一個沒有講出口的顧慮。你如果只等他想清楚，通常就會變成沒有下文。',
        method: '用五連問把「再想想」問到真正原因，再用 MBAF 把他在意的利益講清楚',
        tip: '你可以問：「你想想看的部分，是價格、效果，還是怕買了之後用不到？」',
        next_question: '他說再想想時，你平常有約下一次確認時間，還是通常就停在那邊？'
      };
    }
    const nextQuestion = chooseConsultativeNextQuestion(slots, quote);
    return {
      surface: '這句話其實已經透露一個銷售訊號。',
      deep: `我會先看它跟${product}、${customer}的關係：客戶真正卡住的，可能不是你講得不夠多，而是還沒聽到跟他自己有關的利益。`,
      method: '先用五連問聽出背後原因，再用 BAF 把你的價值翻成客戶聽得懂的好處',
      tip: '你現在不用急著把所有細節補滿，先讓我抓到你的銷售現場就好',
      next_question: nextQuestion
    };
  }

  function chooseConsultativeNextQuestion(slots, quote) {
    if (!slots.product) return '你現在主要賣的是什麼產品或服務？';
    if (!slots.customer) return `買${slots.product}的通常是哪一種客戶？`;
    if (!slots.problem) return `以${slots.customer}來說，最近最常不往前走的理由是什麼？`;
    if (!slots.goal) return '你現在最想先改善哪一件事：成交率、客戶信任、追蹤效率，還是介紹產品時講不清楚？';
    if (/成交|買|下單/.test(quote)) return '如果下次再遇到同樣客戶，你最想先改善的是開場、介紹產品，還是處理價格？';
    return '如果下次再遇到同樣狀況，你最想先改善的是成交、信任、追蹤，還是把價值講清楚？';
  }

  function latestMeaningfulUserQuote(state) {
    const latest = [...(state.messages || [])]
      .reverse()
      .find((msg) => msg.role === 'user' && isConsultativeQuote(msg.content));
    if (!latest) return '';
    const value = stripDisplayText(latest.content).replace(/\s+/g, ' ').trim();
    return value.length > 38 ? value.slice(0, 37) + '...' : value;
  }

  function isConsultativeQuote(text) {
    const value = stripDisplayText(text);
    if (!value) return false;
    if (value.length < 2) return false;
    if (isAgreement(value) || isDisagreement(value) || isCourseCommercialQuestion(value) || isServiceObjection(value)) return false;
    return /[A-Za-z0-9\u4e00-\u9fff]/.test(value);
  }

  function isGenericDiscoveryQuestion(reply) {
    return /(主要賣什麼|賣的是什麼|哪一種客戶|哪種客戶|客戶通常|客戶是誰|會買單的是|最想讓自己哪個地方更強|哪一個地方變得更強|哪個地方變得更強|哪一句話講得更有力量|最想讓這份報告幫你看見哪一個銷售優勢)/.test(reply);
  }

  function isRepeatedFinalScenarioQuestion(reply) {
    return /(最後補一個真實情境|最近一次客戶沒有往前走|原話大概怎麼說|最後再確認一題)/.test(reply);
  }

  function isReplyExactRepeat(state, reply) {
    const lastAssistant = [...(state.messages || [])]
      .reverse()
      .find((msg) => msg.role === 'assistant');
    if (!lastAssistant) return false;
    return normalizeForEvidence(lastAssistant.content) === normalizeForEvidence(reply);
  }

  function replyReferencesQuote(reply, quote) {
    const cleanReply = normalizeForEvidence(reply);
    const tokens = quoteTokens(quote);
    return tokens.some((token) => cleanReply.includes(normalizeForEvidence(token)));
  }

  function quoteTokens(quote) {
    const value = stripDisplayText(quote);
    const known = value.match(/(沒錢|太貴|預算|價格|再比較|比較|家人|另一半|主管|討論|沒有效|效果|擔心|怕買錯|沒時間|很忙|再想想|考慮看看|考慮|保健品|保時捷|企業老闆|上班族|醫美|保險|顧問服務)/g) || [];
    const chunks = value
      .split(/[，。,.、\s]+/)
      .map((item) => item.trim())
      .filter((item) => item.length >= 2 && item.length <= 12);
    return Array.from(new Set(known.concat(chunks)));
  }
  ...
  function buildDeterministicProfileReady(state) {
    ...
    const quote = latestMeaningfulUserQuote(state);
    const quoteLead = quote ? `我有聽到你剛剛講「${quote}」。` : '';
    return {
      phase: PHASES.PROFILE_READY,
      reply: `${state.profile && state.profile.name ? state.profile.name + '，' : ''}${quoteLead}我把你的銷售天賦整理成表格，已經放在底下給你囉。\n\n你覺得有準嗎？也可以繼續了解怎麼做，讓自己成長更快。想繼續了解，直接回 yes。`,
      next_question: '',
  ...
  function learnSlotsFromText(slots, text, expectedSlot) {
    ...
    if (expectedSlot === 'problem' && contextual && (extractProblem(value) || !extractGoal(value))) {
  ...
  function inferAskedSlot(text) {
    ...
    if (/(沒有往前走|沒有成交|沒有買|最常用什麼理由|最常不買|停在價格|停在.*信任|價格高|看不到效果|沒時間|再想想|真正擔心|卡在哪|遇到的狀況)/.test(value)) return 'problem';
  ...
  function extractProduct(text) {
    ...
    const known = value.match(/(企業顧問服務|高價顧問服務|顧問服務|醫美療程|保健品|保時捷|賓士|BMW|特斯拉|汽車|車|房地產|房子|保險|醫美|美容|課程|顧問|軟體|系統|投資|營養品)/);
  ...
  function isAgreement(text) {
    ...
    if (/^(yes|y|ok|okay)$/i.test(value)) return true;
    if (/^(有|有啊|有喔|準|很準|像|像我|有像|有準)$/.test(value)) return true;
    return /(很準|有準|有像|像我|有中|符合|滿對|蠻對|很對|對耶|是我|沒錯|繼續了解)/.test(value);
  }
