...
function validateCourseOfferings(data) {
  ...
  for (const offering of data.offerings || []) {
    ...
    if (offering.status === 'price_confirmed_date_pending') {
      if (offering.date_time || offering.end_date_time) errors.push(`${prefix} price_confirmed_date_pending offering cannot contain scheduled date`);
      if (offering.price === null || offering.price === undefined) errors.push(`${prefix} price_confirmed_date_pending offering requires price`);
      if (!offering.currency) errors.push(`${prefix} price_confirmed_date_pending offering requires currency`);
      if (!offering.venue) errors.push(`${prefix} price_confirmed_date_pending offering requires venue`);
      if (String(data.verified_by).includes('PENDING')) errors.push(`${prefix} price_confirmed_date_pending offering cannot use pending verifier`);
    }
    ...
    offerings: (source.offerings || []).map((offering) => {
      ...
      if (offering.status === 'date_confirmed_price_pending') {
        return {
          ...
          time_confirmed: offering.time_confirmed,
          signup_url: offering.signup_url,
